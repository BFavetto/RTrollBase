...
// internal function to get the number of lines in a file
...
SEXP readl(std::string path, bool lines) {
  ...
  if (lines) {
    ...
    if (strchr(map, '\n') == NULL && strchr(map, '\r') == NULL) {
      ...
      // allocate a vector of strings (R format)
      ...
      while (pch != NULL) {
        ...
        SET_STRING_ELT(output, 
                       i, 
                       Rf_mkCharLen(pch_old, (int)((size_t) pch - (size_t) pch_old)));
